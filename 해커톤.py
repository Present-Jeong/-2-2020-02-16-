...
driver.find_element_by_css_selector('input#oneid').send_keys("ohsku77")

driver.find_element_by_css_selector('#loginform > span > input[type=password]:nth-child(2)').send_keys("didfud4fkd")

driver.find_element_by_css_selector('input.submit').click()
time.sleep(0.5)

driver.find_element_by_css_selector('#header > div.bottom > div > div > ul > li:nth-child(3) > a').click()
time.sleep(1)

driver.find_element_by_css_selector('#m113 > a').click()
driver.find_element_by_css_selector('#sm234 > a').click()

print('---------------------------')
time.sleep(4)
print(driver.find_element_by_css_selector('iframe#_component'))
...
# print(122222)
# driver.switch_to.frame(driver.find_element_by_css_selector('iframe#_component'))

# body > div > div.page > form > div > div.single > div > select:nth-child(5)
# time.sleep(1.5)
# print(3333333333)
print( driver.find_element_by_css_selector("div.single select").text )
#
print(4444)
...
for i in range(0,23) :
    time.sleep(2)
    select_fr = Select(driver.find_element_by_css_selector("body > div > div.page > form > div > div.single > div > select:nth-child(5)"))
    select_fr.select_by_index(i+1)
    driver.find_element_by_css_selector('body > div > div.page > form > div > div.single > span.box_right > input').click()
    time.sleep(1)

    #driver.switch_to.default_content()

    stores = driver.find_elements_by_css_selector("div.page:nth-child(2) tbody> tr")
    for store in stores:
        class_name = store.find_element_by_css_selector('tr > td:nth-child(1)').text
        pro_name = store.find_element_by_css_selector('tr > td:nth-child(5)').text
        print(class_name, pro_name)
        f.write(class_name + "," + pro_name + "\n")

driver.close()
    # driver.find_element_by_css_selector('body > div > div.page > form > div > div.single > span.box_right > input').click()

    #driver.switch_to.default_content() # 메인프레임으로 돌아가기
